...
export const queryOpenAIChat = async (_req, res, next) => {
  ...
  //handle chat completions with GPT
    try {
      const contextMessages = pineconeQueryResult.map(result => 
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a helpful book recommendation assistant. Use the provided book context to answer questions and make recommendations."
          },
          {
            role: "user",
